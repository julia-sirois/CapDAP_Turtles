...
Plot the MCPs
```{r, fig.cap='Figure 1. MCP visual for each turtle'}
plot(utm_sp, col = as.factor(utm_sp@data$Turtle_ID), pch = 16)
plot(turtles_mcp, col = alpha(1:30, 0.5), add = TRUE)
```

Now calculate the MCPs
```{r, message = FALSE, warning = FALSE, fig.cap= Figure 2. Home range sequential increases per individual}
...
### Plot the relationship
```{r, fig.cap= Figure 4. Histogram of distance to stream relocations}
...
```{r, fig.cap= Figure 5. All distance to stream relocations with mean for each sex}
...
### Publication Plot
```{r, fig.cap= Figure 6. Mean Distance to stream per individual (black) with group mean (red)}
